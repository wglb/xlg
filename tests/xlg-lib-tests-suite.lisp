...
(in-package #:xlg-lib-tests) ; Package name changed to :xlg-lib-tests

;; Define the main test suite for XLog
(fiveam:def-suite xlg-lib-tests-suite ; Suite name changed
...
(fiveam:test single-stream-logging-with-prefixes
  ...
  (let ((log-file (concatenate 'string (xlg-lib::dates-ymd :ymd) "single-stream.log")))
    ...
    (xlg-lib:with-open-log-files ((:single-log "single-stream.log" :ymd)) ; Use xlg-lib:with-open-log-files
...
(fiveam:test two-stream-logging-varied-prefixes
  ...
  (let ((app-log-file (concatenate 'string (xlg-lib::dates-ymd :ymd) "two-streams-app.log"))
        (err-log-file "two-streams-error.log")) ; This one doesn't have a date prefix, so it's fine
    (when (probe-file app-log-file) (delete-file app-log-file))
    (when (probe-file err-log-file) (delete-file err-log-file))
    (xlg-lib:with-open-log-files ((:app-log "two-streams-app.log" :ymd) ; Use xlg-lib:with-open-log-files
                                  (:err-log "two-streams-error.log"))
      ;; Line prefixed with microsecond timestamp and a custom string
      (xlg-lib:xlg :app-log "Application event: User login successful for ~a (date-prefixed filename and line)" "Bill" :line-prefix "[APP] ") ; Use xlg-lib:xlg
      ;; Line prefixed with microsecond timestamp, no additional string
      (xlg-lib:xlg :err-log "Error encountered: Failed to connect to database (no filename prefix, but line has microsecond date)." :line-prefix "[SEC-LOG] ") ; Use xlg-lib:xlg
      ;; No line prefix
      (xlg-lib:xlg :app-log "Application event: Data processed successfully (date-prefixed filename, no line prefix)." "date-prefixed filename, no line prefix") ; Use xlg-lib:xlg
      ;; No line prefix
      (xlg-lib:xlg :err-log "Another error message (no filename prefix, no line prefix).")) ; Use xlg-lib:xlg
    ;; Assertions moved here, outside with-open-log-files, but inside the outer let
...
(fiveam:test three-stream-logging-with-prefixes
  ...
  (let ((main-log-file (concatenate 'string (xlg-lib::dates-ymd :ymd) "three-streams-main.log"))
    ...
    (xlg-lib:with-open-log-files ((:main-log "three-streams-main.log" :ymd) ; Use xlg-lib:with-open-log-files
                                  (:debug-log "three-streams-debug.log" :ymd)
                                  (:audit-log "three-streams-audit.log" :ymd))
      (xlg-lib:xlg :main-log "System initialization started (main log, filename & line prefixed)." :line-prefix "[MAIN] ") ; Use xlg-lib:xlg
      (xlg-lib:xlg :debug-log "Debug: Configuration loaded from ~a (debug log, filename & line prefixed)" "/etc/config.ini" :line-prefix "[SEC-LOG] ") ; Use xlg-lib:xlg
      (xlg-lib:xlg :audit-log "Audit: Access granted to IP ~a at ~a (audit log, filename & line prefixed)" "192.168.1.100" (get-universal-time) :line-prefix "[AUDIT] ") ; Use xlg-lib:xlg
      (xlg-lib:xlg :main-log "Processing user request: ~a (main log, filename prefixed, no line prefix)" "fetch_data") ; Use xlg-lib:xlg
      (xlg-lib:xlg :debug-log "Debug: Query executed in ~a ms (debug log, filename prefixed, no line prefix)" 150) ; Use xlg-lib:xlg
      (xlg-lib:xlg :audit-log "Audit: Data read by user ~a (audit log, filename prefixed, no line prefix)" "admin") ; Use xlg-lib:xlg
      (xlg-lib:xlg :main-log "System shutdown initiated (main log, filename & line prefixed)." :line-prefix "[MAIN] ")) ; Use xlg-lib:xlg
    ;; Assertions moved here, outside with-open-log-files, but inside the outer let
...
(fiveam:test append-vs-replace-file-opening
  ...
  (let ((append-file "append-test.log")
    ...
    (xlg-lib:with-open-log-files ((:append-log "append-test.log" nil :append))
    ...
    (xlg-lib:with-open-log-files ((:replace-log "replace-test.log" nil :replace))
