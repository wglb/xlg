...
(fiveam:test single-stream-logging-with-prefixes
  ...
  (let ((log-file (merge-pathnames (xlg-lib::dates-ymd :ymd) "single-stream.log")))
    ...
    (xlg-lib:with-open-log-files ((:single-log "single-stream.log" :ymd)) ; Use keyword for stream
      ...
      ;; Test XLG with :timestamp T
      (let ((returned-string (xlg-lib:xlg :single-log "Message with explicit timestamp." :timestamp t)))
        (fiveam:is-true (search "Message with explicit timestamp." returned-string) "XLG should return string with message.")
      ...
      ;; Test XLG without :timestamp (default NIL)
      (let ((returned-string (xlg-lib:xlg :single-log "Message without timestamp (default).")))
        (fiveam:is-true (search "Message without timestamp (default)." returned-string) "XLG should return string with message.")
    ...
    (let ((content (uiop:read-file-string log-file)))
      ...
      (fiveam:is-true (search "Message with explicit timestamp." content) "Explicit timestamp message should be in log.")
      (fiveam:is-true (search "Message without timestamp (default)." content) "Default no timestamp message should be in log.")
      ;; Verify absence of timestamp for default case
      (fiveam:is-false (search (subseq (xlg-lib::formatted-current-time-micro "") 0 20) (subseq content (search "Message without timestamp (default)." content))) "Timestamp should not be present for default no timestamp line."))
